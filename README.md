...
- **설명**: 팀원들의 **식사 여부(수령/미수령)를 손쉽게 관리**하기 위한 사내용 웹 애플리케이션입니다.  
  날짜·시간대별 식사 스케줄을 등록하고, 구성원들이 실제로 식사를 **가져갈 때 수령 여부를 체크**하면, 관리자는 **식사 수령 인원·기록·통계를 한눈에 확인**할 수 있습니다.
- **목적**
...
- **식사 스케줄 관리 (관리자)**
  ...
  - 스케줄별 생성자, 활성 여부, 식사 수령 인원 수 표시
  - 중복 방지: 같은 날짜·시간대에는 한 개 스케줄만 등록 가능
- **식사 수령 체크 (구성원)**
  - 오늘/특정 날짜에 대한 식사 스케줄 조회
  - 각 스케줄에 대해 실제로 식사를 가져갈 때 **수령/미수령 체크 및 메모 작성**
  - 비활성 사용자(active=false)는 체크 불가
- **식사 이력 & 통계**
  - 개인별 식사 수령 이력(기간 필터) 조회
  - 전체 식사 이력 및 사용자별/기간별 통계 API
  - 스케줄별 총 활성 사용자 수, 식사 수령 인원 수 집계
  - 전체 식사 수령 이력을 화면에서 필터링 후 **CSV(엑셀) 파일로 다운로드** 가능
- **사용자 관리 (관리자)**
  - 사용자 목록/활성 사용자 목록 조회
